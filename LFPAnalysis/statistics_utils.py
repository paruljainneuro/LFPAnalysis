...
def permutation_regression_zscore(data, formula, n_permutations=1000, plot_res=False):
    ...
    # print(original_params.shape, permuted_params.shape)
    # print(np.array(original_params))
    p_nonparametric = np.sum(abs(np.array(original_params)) >= abs(permuted_params), axis=0)/n_permutations
...
def time_resolved_regression_single_channel(timeseries=None, regressors=None, 
                             win_len=100, slide_len=25, n_permutations=500,
...
def fit_mixed_effects_permuted_model(formula, data, permuted_y, y_var):
    """
    Convenience function for running backend Lmer with surrogates
    """
    permuted_data = data.copy()
    permuted_data[y_var] = permuted_y
    return Lmer(formula, data=permuted_data.dropna(inplace=False)).fit(verbose=False)[['Estimate']]
    # p_model['regressor'] = p_model.index
    # return p_model[['regressor', 'Estimate']]

def permutation_mixed_effects_regression_zscore(data, formula, n_permutations=1000, plot_res=False):
    """

    A quick way to perform single-electrode regression with many permutations: 
    # Example usage:
    # data = pd.DataFrame({'y': y, 'x1': x1, 'x2': x2, 'category': ['A', 'B', 'A', 'B', ...]})
    # formula = 'y ~ x1 + x2 + C(category) + (1 + x3/x4)'
    # results = permutation_mixed_effects_regression_zscore(data, formula, plot_res=True)
    # print(results)

    """
    # Perform original regression
    original_model_full = Lmer(formula, data=data.dropna(inplace=False))
    original_model = original_model_full.fit(verbose=False)
    
    # Extract original coefficients
    original_model['regressor'] = original_model.index
    original_params = original_model.Estimate#[['regressor', 'Estimate']]    
    ...
    for _ in tqdm(range(n_permutations), desc="Permutations"):
        y_permuted = np.random.permutation(y_values)
        permuted_params.append(fit_mixed_effects_permuted_model(formula, data, y_permuted, y_var))
        permuted_y_values.append(y_permuted)
    permuted_params = pd.concat(permuted_params)

    # Convert to numpy array for faster computations
    permuted_params['regressor'] = permuted_params.index
    permuted_params['idx'] = permuted_params.groupby('regressor').cumcount()
    permuted_params_wide = permuted_params.pivot_table(
        index='idx',       # The column that becomes the new row index
        columns='regressor',    # The column whose unique values become the new column headers
        values='Estimate'       # The column whose values populate the new DataFrame cells
    )    # Optional: Removes the 'group_id' column and resets to default index
    permuted_params = np.array(permuted_params_wide)
...
def time_resolved_mixed_effects_regression_single_channel(timeseries=None, regressors=None, 
                             controls=None, formula=None, win_len=100, slide_len=25, n_permutations=500,
                             standardize=True, smooth=False, permute=False, sr=500):
    """
    In this function, if you provide a 2D array of z-scored time-varying neural data and a sert of regressors, 
    this function will run a time-resolved generalized linear model with the provided regressor dataframe. 
    ...
    regressors : pandas.DataFrame, dimensions = trials x regressors
        Dataframe containing the regressors.
    controls : pandas.DataFrame, dimensions = trials x controls
        Dataframe containing the controls.
    win_len : int
        Length of the window for the time-resolved regression.
    slide_len : int
        Step size for the time-resolved regression.
    ...
    if standardize:
        regressors = regressors.apply(lambda x: (x - np.nanmean(x))/(2*np.nanstd(x)))
        # controls = controls.apply(lambda x: (x - np.nanmean(x))/(2*np.nanstd(x)))
    ...
    # write the regression formula
    if formula == None:
        fixed_effects = f'1+'+'+'.join(regressors.keys())
        random_effects = '+'.join(controls.keys())
        formula = f'sig ~ +{fixed_effects}+({fixed_effects}|{random_effects})'
    for ts in range(sig.shape[-1]):
        regressors['sig'] = sig[:, ts]
        for c in controls.keys():
            regressors[c] = controls[c]
        if permute:
            results = permutation_mixed_effects_regression_zscore(regressors,
            formula,
            n_permutations=n_permutations, 
            plot_res=False)
            results['ts'] = ts
        else:
            original_model_full = Lmer(formula, data=regressors.dropna(inplace=False))
            original_model = original_model_full.fit()
            random_effects_var = {}
            random_effects_std = {}
            for c_key in original_model_full.ranef_var.index:
                random_effects_var[c_key+'_Var'] = original_model_full.ranef_var.loc[c_key]['Var']
